...
class AdaptiveOpticsDevice(Device):
    ...
    def __init__(self, ao_element_uri, wavefront_uri=None, slm_uri=None, **kwargs):
        ...
        self.ao_element = Pyro4.Proxy(
            "PYRO:%s@%s:%d"
            % (ao_element_uri[0].__name__, ao_element_uri[1], ao_element_uri[2])
        )
        # Wavefront sensor. Must support soft_trigger for now.
        if wavefront_uri is not None:
            self.wavefront_camera = Pyro4.Proxy(
                "PYRO:%s@%s:%d"
                % (wavefront_uri[0].__name__, wavefront_uri[1], wavefront_uri[2])
            )
        # SLM device
    ...
    def generate_isosense_pattern_image(self, shape, dist, wavelength, NA, pixel_size):
        try:
            ...
            raise Exception(
                "Expected %s instead recieved %s" % (type((512, 512)), type(shape))
            )
        ...
        try:
            ...
            raise Exception(
                "Expected tuple of length 2, instead recieved length %i" % len(shape)
            )
    ...
    def apply_isosense_pattern(self, fill_frac, wavelength, NA, pixel_size):
        ...
        pattern = self.generate_isosense_pattern_image(
            shape=shape, wavelength=wavelength, dist=dist, NA=NA, pixel_size=pixel_size
        )
    ...
    def set_pupil_ac(self, pupil_ac):
        try:
            ...
            raise Exception(
                "Length mismatch between pupil mask (%i) and "
                "number of actuators (%i). Please provide a mask "
                "of the correct length" % (np.shape(pupil_ac)[0], self.numActuators)
            )
    ...
    def set_fourierfilter(self, test_image, region=None, window_dim=None, mask_di=None):
        ...
        try:
            self.fft_filter = aoAlg.make_fft_filter(
                test_image, region=region, window_dim=window_dim, mask_di=mask_di
            )
        except Exception as e:
            _logger.info(e)
        return self.fft_filter
    ...
    def createcontrolmatrix(
        self, imageStack, noZernikeModes, pokeSteps, pupil_ac=None, threshold=0.005
    ):
        # Ensure an ROI is defined so a masked image is obtained
        try:
            assert np.any(self.roi) is not None
            ...
            if pupil_ac[ac] == 1:
                ...
                for im in range(numPokeSteps):
                    ...
                    if no_discontinuities > (x * y) / 1000.0:
                        ...
                        _logger.info(
                            "Zernike modes %d/%d not calculated" % (curr_calc, noImages)
                        )
                    else:
                        pokeAc[ac] = pokeSteps[im]
                        all_pokeAmps.append(pokeAc.tolist())
                        _logger.info(
                            "Calculating Zernike modes %d/%d..." % (curr_calc, noImages)
                        )
    ...
    def wavefront_strehl_ratio(self, phase_map=None):
        ...
        strehl_ratio = np.exp(
            -np.mean((phase_map[self.mask] - np.mean(phase_map[self.mask])) ** 2)
        )
        return strehl_ratio
    ...
    def flatten_phase(self, iterations=1, error_thresh=np.inf, z_modes_ignore=None):
        ...
        while (iterations > ii) or (best_error > error_thresh):
            ...
            correction_wavefront_mptt = (
                correction_wavefront - aotools.phaseFromZernikes(z_amps[0:3], x)
            )
            ...
            ## Here we ignore piston, tip and tilt since they are not true optical aberrations
            corrected_wavefront_mptt = corrected_wavefront - aotools.phaseFromZernikes(
                z_amps_corrected[0:3], x
            )
    ...
    def correct_sensorless_single_mode(
        ...
        amp_to_correct = aoAlg.find_zernike_amp_sensorless(
            image_stack,
            zernike_applied,
