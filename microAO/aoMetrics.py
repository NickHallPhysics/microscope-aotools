...
def make_OTF_mask(size, inner_rad, outer_rad):
    ...
    outer_mask = (
        np.sqrt(
            (np.arange(-rad_y, rad_y) ** 2).reshape((rad_y * 2, 1))
...
def measure_fourier_metric(
    image, wavelength, NA, pixel_size, noise_amp_factor=1.125, **kwargs
):
    ...
    tukey_window = tukey(np.max(im_shift.shape), 0.10, True)
    tukey_window = np.fft.fftshift(
        tukey_window.reshape(1, -1) * tukey_window.reshape(-1, 1)
    )
    tukey_window_crop = tukey_window[
        int(tukey_window.shape[0] / 2 - im_shift.shape[0] / 2) : int(
            tukey_window.shape[0] / 2 + im_shift.shape[0] / 2
        ),
        int(tukey_window.shape[1] / 2 - im_shift.shape[1] / 2) : int(
...
def measure_gradient_metric(image, **kwargs):
    ...
    correction_grad = np.sqrt(
        (image_gradient_x * grad_mask_x) ** 2 + (image_gradient_y * grad_mask_y) ** 2
    )
...
def measure_fourier_power_metric(
    image,
    wavelength,
    NA,
    pixel_size,
    noise_amp_factor=1.125,
    high_f_amp_factor=100,
    **kwargs
):
    ray_crit_dist = (1.22 * wavelength) / (2 * NA)
    ray_crit_freq = 1 / ray_crit_dist
    ...
    high_f_amp_mask = (
        high_f_amp_factor * (ramp_mask * omega) / np.max(ramp_mask * omega)
    )
...
def measure_second_moment_metric(image, wavelength, NA, pixel_size, **kwargs):
    ...
    metric = np.sum(ring_mask * fftarray_sq_log * ramp_mask * omega) / np.sum(
        fftarray_sq_log
    )
    return metric
