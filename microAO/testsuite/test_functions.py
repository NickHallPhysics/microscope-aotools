...
class TestAOFunctions(unittest.TestCase):
    def _gaussian_funcion(self, x, offset, normalising, mean, std_dev):
        return (offset - normalising) + (
            normalising * np.exp((-((x - mean) ** 2)) / (2 * std_dev ** 2))
        )
    ...
    def _construct_true_mask(self):
        ...
        mask = (
            np.sqrt(
                (np.arange(-self.radius, self.radius) ** 2).reshape((diameter, 1))
                + (np.arange(-self.radius, self.radius) ** 2)
            )
            < self.radius
        )
        return mask
    ...
    def _construct_single_mode_measurements(
        self, shape, z_min, z_max, num_mes, true_max
    ):
        ...
        for ii in range(num_mes):
            stack[ii, min_y:max_y, min_x:max_x] = self._gaussian_funcion(
                z_measurements[ii], 100, 100, true_max, ((z_max - z_min) / 4)
            )

        return stack

    def _construct_multiple_mode_measurements(
        self, shape, z_min, z_max, num_mes, all_true_max, noll_zernike
    ):
        ...
        for ii in range(len(noll_zernike)):
            for jj in range(num_mes):
                stack[
                    jj + (num_mes * ii), min_y:max_y, min_x:max_x
                ] = self._gaussian_funcion(
                    z_measurements[jj],
                    100,
                    100,
    ...
    def setUp(self):
        ...
        self.AO_fft_filter = self.AO_func.make_fft_filter(
            image=self.test_inter, region=None
        )
        self.true_ac_applied = np.linspace(0, 1, self.nzernike)
        self.true_metric_single_measure = np.outer(
            gaussian(100, 10), gaussian(100, 10).T
        )
        ...
        self.true_single_mode_measurements = self._construct_single_mode_measurements(
            (100, 100),
            self.true_z_min,
            self.true_z_max,
            self.true_num_mes,
        ...
        self.true_multi_mode_measurements = self._construct_multiple_mode_measurements(
            (100, 100),
            self.true_z_min,
            self.true_z_max,
            self.true_num_mes,
    ...
    def test_fourier_filter(self):
        test_fft_filter = self.AO_func.make_fft_filter(
            image=self.test_inter, region=None
        )

        true_pos = np.asarray([self.true_y_freq, self.true_x_freq])
        max_pos = abs(
            np.asarray(np.where(test_fft_filter == np.max(test_fft_filter))) - 1024
        )
    ...
    def test_unwrap_interferometry(self):
        ...
        zcoeffs_in[2] = 1
        aberration_angle = aotools.phaseFromZernikes(
            zcoeffs_in, self.test_inter.shape[1]
        )
        aberration_phase = (
            1 + np.cos(aberration_angle) + (1j * np.sin(aberration_angle))
        ) * self.true_mask
        test_phase = self.test_inter * aberration_phase
        aberration = unwrap_phase(
            np.arctan2(aberration_phase.imag, aberration_phase.real)
        )
        ...
        ab_ratio_mean = np.mean(
            test_aberration[aberration != 0] / aberration[aberration != 0]
        )
        ab_ratio_var = np.var(
            test_aberration[aberration != 0] / aberration[aberration != 0]
        )
    ...
    def test_createcontrolmatrix(self):
        ...
        test_control_matrix = self.AO_func.create_control_matrix(
            zernikeAmps=allTestZernikeAmps,
    ...
    def test_ac_pos_from_zernike(self):
        ...
        test_ac_pos = self.AO_func.ac_pos_from_zernike(
            test_zernike_applied, self.planned_n_actuators
        )
    ...
    def test_measure_metric_fourier(self):
        ...
        test_fourier_metric = self.AO_func.measure_metric(
            self.true_metric_single_measure,
            wavelength=self.test_wavelength,
            NA=self.test_NA,
            pixel_size=self.test_pixel_size,
        )
        np.testing.assert_almost_equal(
            test_fourier_metric / self.true_fourier_metric, 1, decimal=2
        )

    def test_measure_metric_fourier_power(self):
        self.AO_func.set_metric("fourier_power")
        test_fourier_power_metric = self.AO_func.measure_metric(
            self.true_metric_single_measure,
            wavelength=self.test_wavelength,
            NA=self.test_NA,
            pixel_size=self.test_pixel_size,
        ...
        np.testing.assert_almost_equal(
            test_fourier_power_metric / self.true_fourier_power_metric, 1, decimal=2
        )

    def test_measure_metric_contrast(self):
        self.AO_func.set_metric("contrast")
        test_contrast_metric = self.AO_func.measure_metric(
            self.true_metric_single_measure
        )
        np.testing.assert_almost_equal(
            test_contrast_metric / self.true_contrast_metric, 1, decimal=2
        )

    def test_measure_metric_gradient(self):
        self.AO_func.set_metric("gradient")
        test_gradient_metric = self.AO_func.measure_metric(
            self.true_metric_single_measure
        )
        np.testing.assert_almost_equal(
            test_gradient_metric / self.true_gradient_metric, 1, decimal=2
        )

    def test_measure_metric_second_moment(self):
        self.AO_func.set_metric("second_moment")
        test_second_moment_metric = self.AO_func.measure_metric(
            self.true_metric_single_measure,
            wavelength=self.test_wavelength,
            NA=self.test_NA,
            pixel_size=self.test_pixel_size,
        ...
        np.testing.assert_almost_equal(
            test_second_moment_metric / self.true_second_moment_metric, 1, decimal=2
        )

    def test_find_zernike_amp_sensorless(self):
        self.AO_func.set_metric("contrast")
        zernike_amplitudes = np.linspace(
            self.true_z_min,
            self.true_z_max,
            self.true_num_mes,
        )
        amplitude_present = self.AO_func.find_zernike_amp_sensorless(
            self.true_single_mode_measurements,
            zernike_amplitudes,
            wavelength=self.test_wavelength,
            NA=self.test_NA,
            pixel_size=self.test_pixel_size,
        ...
        np.testing.assert_almost_equal(
            -1 * amplitude_present, self.true_max_mode_z, decimal=2
        )

    def test_get_zernike_modes_sensorless(self):
        self.AO_func.set_metric("contrast")
        z_steps = np.linspace(self.true_z_min, self.true_z_max, self.true_num_mes)
        full_zernike_applied = np.zeros(
            (
                self.true_num_mes * self.true_noll_zernike.shape[0],
        ...
        for noll_ind in self.true_noll_zernike:
            ...
            full_zernike_applied[
                ind * self.true_num_mes : (ind + 1) * self.true_num_mes, noll_ind - 1
            ] = z_steps

        coef = self.AO_func.get_zernike_modes_sensorless(
            self.true_multi_mode_measurements,
            full_zernike_applied,
            self.true_noll_zernike,
            wavelength=self.test_wavelength,
            NA=self.test_NA,
        ...
        for noll_ind in self.true_noll_zernike:
            np.testing.assert_almost_equal(
                -1 * coef[noll_ind - 1], self.true_max_modes_z[noll_ind - 1], decimal=2
            )
