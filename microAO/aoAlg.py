...
## Copyright (C) 2018 Nicholas Hall <nicholas.hall@strath.ac.uk>
## Copyright (C) 2017 Josh Edwards <Josh.Edwards222@gmail.com>
## Copyright (C) Jacopo Antonello <jacopo.antonello@dpag.ox.ac.uk>
##
...
class AdaptiveOpticsFunctions:
    ...
    def set_metric(self, metric):
        if metric in metric_function.keys():
            ...
            raise Exception(
                "Error: %s is not a supported image quality metric" % metric
            )
        return
    ...
    def make_mask(self, radius):
        ...
        self.mask = (
            np.sqrt(
                (np.arange(-radius, radius) ** 2).reshape((diameter, 1))
                + (np.arange(-radius, radius) ** 2)
            )
            < radius
        )
        return self.mask
    ...
    def make_fft_filter(self, image, region=None, window_dim=None, mask_di=None):
        ...
        tukey_window = tukey(fringes.shape[0], 0.10, True)
        tukey_window = np.fft.fftshift(
            tukey_window.reshape(1, -1) * tukey_window.reshape(-1, 1)
        )
        ...
        fftarray[
            find_cent[1] - region : find_cent[1] + region,
            find_cent[0] - region : find_cent[0] + region,
        ] = (
            0.00001 + 0j
        )
        ...
        test_point = [
            int(test_point % fftarray.shape[1]),
            int(test_point / fftarray.shape[1]),
        ]
            ...
            try:
                window[:, :] = np.log(
                    abs(
                        fftarray[
                            maxpoint[1]
                            - window_dim // 2 : maxpoint[1]
                            + window_dim // 2,
                            maxpoint[0]
                            - window_dim // 2 : maxpoint[0]
                            + window_dim // 2,
                        ]
                    )
                )
                ...
                raise Exception(
                    "Interferometer stripes are too fine. Please make them coarser or reduce window size"
        ...
        x_shift = np.min(
            (
                0,
                abs(maxpoint[0] - mask_di),
                abs(maxpoint[0] - fftarray.shape[0]) - mask_di,
            )
        )
        y_shift = np.min(
            (
                0,
                abs(maxpoint[1] - mask_di),
                abs(maxpoint[1] - fftarray.shape[1]) - mask_di,
            )
        )
    ...
    def unwrap_interferometry(self, image, **kwargs):
        ...
        centre_y_array, centre_x_array = np.where(
            self.fft_filter == np.max(self.fft_filter)
        )
        g1 = int(
            np.round(np.mean(centre_y_array)) - np.round(fftarray_filt.shape[0] // 2)
        )
        g0 = int(
            np.round(np.mean(centre_x_array)) - np.round(fftarray_filt.shape[0] // 2)
        )
    ...
    def get_zernike_modes(self, image_unwrap, noZernikeModes, resize_dim=128):
        ...
        image_resize = self.bin_ndarray(
            image_unwrap, new_shape=(resize_dim, resize_dim), operation="mean"
        )
        ...
        for i in range(1, (noZernikeModes + 1)):
            intermediate = trapz(
                image_resize * aotools.zernike.zernike_noll(i, resize_dim)
            )
            zcoeffs_dbl.append(trapz(intermediate) / (num_pixels))
        coef = np.asarray(zcoeffs_dbl)
        return coef

    def create_control_matrix(
        self, zernikeAmps, pokeSteps, numActuators, pupil_ac=None, threshold=0.005
    ):
        ...
        for ii in range(numActuators):
            if pupil_ac[ii] == 1:
                ...
                if len(pokeSteps_trimmed) < 2:
                    raise Exception(
                        "Not enough Zernike mode values to calculate slope for actuator %i. "
    ...
    def ac_pos_from_zernike(self, applied_z_modes, numActuators):
        if int(np.shape(applied_z_modes)[0]) < int(np.shape(self.controlMatrix)[1]):
            pad_length = int(np.shape(self.controlMatrix)[1]) - int(
                np.shape(applied_z_modes)[0]
            )
    ...
    def find_zernike_amp_sensorless(self, image_stack, zernike_amplitudes, **kwargs):
        ...
        for ii in range(image_stack.shape[0]):
            ...
            metric_measured = metric_function[self.metric](
                image_stack[ii, :, :], **kwargs
            )
        ...
        z_l_bound = np.min(zernike_amplitudes) - (
            0.25 * (np.max(zernike_amplitudes) - np.min(zernike_amplitudes))
        )
        z_u_bound = np.max(zernike_amplitudes) + (
            0.25 * (np.max(zernike_amplitudes) - np.min(zernike_amplitudes))
        )
        try:
            [offset, normalising, mean, std_dev], pcov = curve_fit(
                gaussian_funcion,
                zernike_amplitudes,
                metrics_measured,
                bounds=(
            ...
            if max_from_mean_var >= 2 * np.sqrt(np.var(metrics_measured)):
                ...
                print(
                    "Could not accurately fit metric polynomial. Defaulting to 0 amplitude."
                )
                mean = 0
    ...
    def get_zernike_modes_sensorless(
        self, full_image_stack, full_zernike_applied, nollZernike, **kwargs
    ):
        ...
        for ii in range(nollZernike.shape[0]):
            ...
            zernike_applied = full_zernike_applied[
                ii * numMes : (ii + 1) * numMes, nollZernike[ii] - 1
            ]
            print(
                "Calculating Zernike amplitude %i/%i" % (ii + 1, nollZernike.shape[0])
            )
            amp = self.find_zernike_amp_sensorless(
                image_stack, zernike_applied, **kwargs
            )
